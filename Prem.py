...
{P2}⣿⣿⣿⣿⡇⠀⠀⢾⣿⣿⣿⣿⣿⣿⡷⠀⠀⢸⣿⣿⣿⣿         
{P2}⣿⣿⣿⣿⣿⡀⠀⠘⢿⣿⣿⣿⣿⡿⠃⠀⢀⣿⣿⣿⣿⣿         {P2}[+] Author Script [ Saya Tidak Tau ]
{P2}⣿⣿⣿⣿⣿⣷⣄⠀⠀⠉⠙⠋⠉⠀⠀⣠⣾⣿⣿⣿⣿⣿         {P2}[+] Recode By BrayennnXD
{P2}⣿⣿⣿⣿⣿⣿⣿⣷⣦⣤⣀⣀⣤⣴⣾⣿⣿⣿⣿⣿⣿⣿         {P2}[+] Nama Script BrayennnIG
...
class instagram:
	...
	def hapus_lisensi(self):
		...
		if ask in ["y","Y"]:os.system("rm -rf data/lisensi.txt");prints(Panel(f"{P2}Succeed Menghapus {color_rich}'Lisensi'{P2} Terima Kasih Telah Menggunakan Script Brayennn-Prem",width=100,style=f"{color_table}"));time.sleep(2);exit()
...
def key():
	...
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIzNTcyOTA3MCIsIndQYk4zUFZjR0ZQUTJEcnhTOHNoSWJQL3BTbGl0UlhES0JGMXFYVUQiXQ==&ProductId=18514&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}selamat lisensi kamu aktif, tolong gunakan tools ini dengan bijak",width=100,padding=(0,4),style=f"{color_table}"));time.sleep(4);LoginCuyy()
	except KeyError:
		prints(Panel(f"{P2}Lisensi Kamu Sudah Kedaluwarsa Silahkan Beli Lisensi Ke Admin",width=100,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+6289622155832?text=Selamat+Pagi+Bang+Mau+Beli+Lisensi+Crack+Instagram");time.sleep(2);exit()

def cek():
	try:x=open("data/lisensi.txt","r").read()
	except FileNotFoundError:key()
	try:x = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyIzNTcyOTA3MCIsIndQYk4zUFZjR0ZQUTJEcnhTOHNoSWJQL3BTbGl0UlhES0JGMXFYVUQiXQ==&ProductId=18514&Key=%s"%(x)).json()['licenseKey']['key'];LoginCuyy()
	except KeyError:
		prints(Panel(f"{P2}Lisensi Kamu Sudah Kedaluwarsa Silahkan Beli Lisensi Ke Admin",width=100,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+6289622155832?text=Selamat+Pagi+Bang+Mau+Beli+Lisensi+Crack+Instagram");time.sleep(2);exit()
...
def Key():
	os.system("clear") 
	banner()
	prints(Panel(f"{P2}Silahkan Ketik {H2}'Beli'{P2} Untuk Melihat Harga Lisensi Tools",width=100,padding=(0,9),style=f"{color_table}"))
	key = input(f" [+] Masukkan Lisensi :{H} ")
	if key in ["beli","Beli","BELI"]:beli_bang()
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIzNTcyOTA3MCIsIndQYk4zUFZjR0ZQUTJEcnhTOHNoSWJQL3BTbGl0UlhES0JGMXFYVUQiXQ==&ProductId=18514&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}Selamat Lisensi Kamu Aktif, Tolong Gunakan Tools Ini Dengan Bijak",width=100,padding=(0,4),style=f"{color_table}"));time.sleep(4);LoginCuyy()
	except KeyError:
		prints(Panel(f"{P2} Lisensi Yang Anda Masukan Tidak Terdaftar Silahkan Beli Terlebih Dahulu",width=100,padding=(0,1),style=f"{color_table}"));os.system("xdg-open https://wa.me/+6289622155832?text=Selamat+Pagi+Bang+Mau+Beli+Lisensi+Crack+Instagram");time.sleep(2);exit()
...
def beli_bang():
    prints(Panel(f"{P2}[{color_rich}01{P2}]. Lisensi 1 Minggu 20K\n{P2}[{color_rich}02{P2}]. Lisensi 2 Minggu 40K\n{P2}[{color_rich}03{P2}]. Lisensi 1 Bulan 80K\n{P2}[{color_rich}04{P2}]. OpenSource 120K\n{P2}[{color_rich}00{P2}]. Keluar ( {M2}Tools{P2} )",width=100,padding=(0,22),style=f"{color_table}"))
    zxc = input(" [+] Pilih Lisensi : ")
    if zxc in [""]:prints(Panel(f"{P2}Pilih Yang Bener Broo Jangan Kosong !",width=100,padding=(0,19),style=f"{color_table}"));time.sleep(3);buy_lisenn()
    elif zxc in ["1","01"]:os.system("xdg-open https://wa.me/+6289622155832?text=Selamat+Pagi+Bang+Mau+Beli+Lisensi+1+Minggu");time.sleep(2);exit()
    elif zxc in ["2","02"]:os.system("xdg-open https://wa.me/+6289622155832?text=Selamat+Pagi+Bang+Mau+Beli+Lisensi+2+Minggu");time.sleep(2);exit()
    elif zxc in ["3","03"]:os.system("xdg-open https://wa.me/+6289622155832?text=Selamat+Pagi+Bang+Mau+Beli+Lisensi+1+Bulan");time.sleep(2);exit()
    elif zxc in ["4","04"]:os.system("xdg-open https://wa.me/+6289622155832?text=Selamat+Pagi+Bang+Mau+Beli+Sc+OpenSource+Nya");time.sleep(2);exit()
    elif zxc in ["0","00"]:time.sleep(2);exit()
    else:prints(Panel(f"{P2}Ngetik Apaan Ngab !",width=100,padding=(0,28),style=f"{color_table}"));time.sleep(3)  
